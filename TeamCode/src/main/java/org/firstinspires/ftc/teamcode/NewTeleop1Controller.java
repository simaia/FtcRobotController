...
@TeleOp(name = "NewTeleop1Controller", group = "PROD")
public class NewTeleop1Controller extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            if (gamepad1.left_bumper) { //merge in sus
                motorBratBaza.setPower(0.5);
            } else if (gamepad1.right_bumper) { //merge in jos
                motorBratBaza.setPower(-0.5);
            } else if (gamepad1.dpad_down) { //merge puternic in jos (atarnare)
            ...
            if (gamepad1.left_trigger > 0) {
                motorBratSus.setPower(gamepad1.left_trigger); //motor brat sus merge upwARDS
            } else if (gamepad1.right_trigger > 0){
                motorBratSus.setPower(-gamepad1.right_trigger); //motor brat sus merge downwaRDS
            } else {
                motorBratSus.setPower(0);
            }

            if (gamepad1.x) { //se inchide
                servoIntake.setPower(0.5);
            } else if (gamepad1.b) { //se deschide
                servoIntake.setPower(-0.5);
            } else {
